...
export function ProviderAccountCreatePage() {
  ...
  const create = useMutation({
    mutationFn: async () => requireData(await createProviderAccount({ body: { provider_id: providerId, display_name: displayName.trim(), tenant: tenant.trim() || null } })),
  ...
  return <PageShell title="添加平台账号" description="先创建 owner-scoped 账号，再通过 Provider 支持的原生认证方法提交凭据。">
    ...
    <Card className="max-w-2xl"><CardHeader><CardTitle>账号信息</CardTitle></CardHeader><CardContent>
      <form className="space-y-4" onSubmit={submit}>
        <div className="space-y-2"><Label htmlFor="provider">Provider</Label><select id="provider" className={selectClassName} value={providerId} onChange={(event) => setProviderId(event.target.value)}>{providers.result.data?.map((provider) => <option key={provider.id} value={provider.id}>{provider.display_name} · {provider.verification}</option>)}</select></div>
        ...
        <div className="space-y-2"><Label htmlFor="tenant">租户（可选）</Label><Input id="tenant" maxLength={256} value={tenant} onChange={(event) => setTenant(event.target.value)} /></div>
