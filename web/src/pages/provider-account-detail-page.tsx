...
export function ProviderAccountDetailPage() {
  ...
  return <PageShell title={account.data.display_name} description={providerName(account.data.provider_id)} actions={<div className="flex flex-wrap gap-2"><Button variant="outline" disabled={scan.isPending || account.data.auth_state.state !== "authenticated"} onClick={() => scan.mutate()}><Search className="size-4" />{scan.isPending ? "正在同步…" : "同步课程与任务"}</Button><Button variant="destructive" disabled={remove.isPending} onClick={() => remove.mutate()}><Trash2 className="size-4" />{remove.isPending ? "删除中…" : "删除账号"}</Button></div>}>
    ...
    {account.data.provider_id === "welearn" ? <Card><CardHeader><CardTitle>WELearn 课程批量执行</CardTitle></CardHeader><CardContent className="space-y-4"><Alert><AlertTitle>从巡查结果直接选择</AlertTitle><AlertDescription>系统会使用当前账号凭据巡查课程和学习任务，并自动填充内部标识与任务数量。标记为“只读”的新增课程只同步数据，不会写入平台。</AlertDescription></Alert>{courses.query.isLoading || selectedBatchTasks.query.isLoading ? <TableSkeleton /> : <div className="grid gap-4 md:grid-cols-2"><Field label="课程"><select className={selectClassName} value={batchCourseId} onChange={(event) => setBatchCourseId(event.target.value)}><option value="">请选择课程</option>{courses.result.data?.map((course) => <option key={course.id} value={course.id} disabled={courseReadOnly(course)}>{course.title}{courseReadOnly(course) ? "（只读）" : ""}</option>)}</select></Field><Field label="已发现任务"><div className="flex h-9 items-center rounded-md border border-input bg-muted px-3 text-sm">{selectedBatchCourseReadOnly ? "只读课程，不会执行" : selectedBatchTasks.result.total ? `${selectedBatchTasks.result.total} 个任务，将在执行前重新核对` : "当前课程尚未发现任务"}</div></Field><Field label="执行方式"><select className={selectClassName} value={batchFlow} onChange={(event) => setBatchFlow(event.target.value as typeof batchFlow)} disabled={selectedBatchCourseReadOnly}><option value="fanyuchang_duration">逐任务计时并完成</option><option value="auto_duration">按总时长自动分配并完成</option></select></Field>{batchFlow === "fanyuchang_duration" ? <Field label="每个任务目标秒数"><Input type="number" min={1} value={batchPerTaskSeconds} onChange={(event) => setBatchPerTaskSeconds(event.target.value)} disabled={selectedBatchCourseReadOnly} /></Field> : <><Field label="总时长（分钟）"><Input type="number" min={1} max={300} value={batchConfiguredMinutes} onChange={(event) => setBatchConfiguredMinutes(event.target.value)} disabled={selectedBatchCourseReadOnly} /></Field><Field label="随机浮动范围（分钟）"><Input type="number" min={0} max={30} value={batchRandomRange} onChange={(event) => setBatchRandomRange(event.target.value)} disabled={selectedBatchCourseReadOnly} /></Field><Field label="本次固定偏移（分钟）"><Input type="number" min={-30} max={30} value={batchSampledOffset} onChange={(event) => setBatchSampledOffset(event.target.value)} disabled={selectedBatchCourseReadOnly} /></Field></>}</div>}{batchResult ? <Alert><AlertTitle>批量执行已调度</AlertTitle><AlertDescription>{batchResult}</AlertDescription></Alert> : null}<Button disabled={createBatch.isPending || !selectedBatchCourse || selectedBatchCourseReadOnly || !(selectedBatchTasks.result.total ?? 0)} onClick={() => createBatch.mutate()}><RefreshCw className="size-4" />{createBatch.isPending ? "调度中…" : "创建课程批量执行"}</Button></CardContent></Card> : null}
...
function courseReadOnly(course: Course | undefined): boolean { return Boolean(course?.metadata && typeof course.metadata === "object" && (course.metadata as Record<string, unknown>).read_only === true); }
