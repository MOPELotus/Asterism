...
| Discussion topic/reply/readback then Group completion | Provider-private two-mutation workflow | Implemented to the Provider boundary; Core fixed topology now fits the verified dependency, while shared immutable reply Draft/artifact/capability persistence remains missing |
| CMS grant, Qiniu upload and single/compound upload submit/readback | Provider-private upload workflows | Implemented to the Provider boundary; Core's receipt-conditional sequence now models the stage-advance rules without treating a receipt as independent verification, while its hash-only receipt cannot recover the exact returned object key. Shared owner/account/Task-bound input artifact, encrypted Provider stage output and single/compound Draft/Attempt ownership remain missing |
...
| Audio/video/subtitle extraction and local Whisper | bounded media manifest/transcript parsing | Provider parsing and response-bound downloaded VTT/SRT normalization are implemented; authenticated downloader, ffmpeg/Whisper execution and model/cache policy are shared gaps |
