...
- Provider metadata and Authentication expose the same validated Capture recipe v3: `AssistedSession` reads same-snapshot `u-openid`, raw `Authorization` and the exact `ucontent` Cookie header into required `ProviderCompositeSession` JSON and purpose-bound `ProviderCookie` outputs. Stored-session fixtures prove the Cookie is optional for native sessions, account/reference bound when present, zeroized, injected with donor client headers only on `ucontent`, and never forwarded to `uai`/`ucloud`; ManualImport, CaptureTool and BrowserExtension remain validated non-renewable `Jwt` imports.
