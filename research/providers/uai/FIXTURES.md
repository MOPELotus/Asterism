...
- Browser residence execution fixtures remain required as its shared executor lands; their absence is a tracked implementation gap, not a first-batch deferral. Exit-ticket, exact single-oral-family and single-upload final mutation boundaries are covered offline, pending live validation; mixed oral/upload compound shapes remain active work.
- Upload boundary coverage requires fresh exact Group hierarchy rebinding, one unique positional `multiFileUpload` module, a Task-fingerprint/artifact-digest-bound intent, code-200 grant parsing, redacted/zeroizing token ownership, exact file-key retention, grant/artifact equality before multipart emission, the donor's 4 KiB minimal MP3, bounded audio/mpeg artifacts, deterministic multipart framing and exact key equality in object-store readback. The returned uploaded-artifact owner retains Task fingerprint, Course/Unit/Group/module/object-key/digest/intent binding and redacts the route. Single-upload final-plan coverage re-reads the same Task, requires position 1 and a fresh positive publish version, emits the donor `instanceId=0` key child inside the minimal current envelope, and rejects the compound upload shape rather than splitting its answers. Native mutation separately rebinds Course instance and an exact incomplete available task progress leaf. Receipt-versioned readback must repeat Group/version, module 0, submitted context, one completed child and the exact object key; changed key/identity/cardinality and missing receipt fail closed, and even a confirmed key still requires fresh progress. Foreign Task identities, duplicate upload modules, artifact swaps and changed returned keys fail before producing it.
- Discussion synthetic coverage binds fresh Course/class/curricula/current-user facts, a unique first-page topic, bounded 20-row reply pages to the exact requested topic, a redacted/zeroizing immutable reply intent, accepted-but-unverified reply receipts and exact current-user/content readback. Only the exact topic/user/content page plus a fresh exact single-discussion Task authorizes the separate completion plan; native completion performs fresh task-progress preflight and one empty `submitType=2` POST, whose receipt still requires fresh progress verification. Foreign topic/content, compound Task and identity drift fail before the second mutation. Durable cross-mutation Draft/attempt registration remains shared integration work.
