...
| Preset completion is forced through ordinary retrying TaskExecution | Donors complete study-mode Groups with a no-Question `submitType=2` POST, while `SubmissionDraft` requires at least one Question | Do not register ResourceExecution until the shared contract can persist the mutation attempt and recover through progress verification only, without replaying the POST |
