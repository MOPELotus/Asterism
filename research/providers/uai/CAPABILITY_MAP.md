...
| Discussion | AutoFinish | Reference | Provider-private native boundaries cover fresh Course/class/curricula/current-user binding, exact topic lookup, bounded route-topic-bound reply pagination, zeroizing immutable reply intent, reply mutation receipt and exact current-user/content readback. Only that readback plus one fresh exact single-discussion Task can freeze the second-mutation plan; native completion then rebinds Course instance and an exact incomplete available task-progress leaf before one empty `submitType=2` POST. Reply and Group receipts remain independent and neither claims completion; fresh TaskProgressRead remains the final verifier. Cross-Provider registration and durable recovery across both non-idempotent mutations still require the shared immutable discussion Draft/attempt contract |
...
| File upload | AutoFinish | Reference | Provider-private native boundaries cover fresh exact Task-detail upload intent, CMS grant acquisition, bounded multipart object upload, final single-`multiFileUpload` mutation and receipt-versioned user-module readback. Task fingerprint, Course/Unit/Group, unique module position, object key, artifact digest and intent fingerprint remain bound across all stages; after object upload, a second fresh Task detail must match exactly and supply the current Course publish version. The final native POST rebinds Course instance and exact incomplete available Unit-progress leaf and uses donor `instanceId=0` with one key-valued completed child. The mixed `multichoice,multiFileUpload` path now independently accepts one validated ordinary sub-draft plus the bound uploaded artifact, fresh-requires that exact ordered two-module Task, builds one atomic two-answer mutation, and verifies both ordered values through the single receipt route. Neither receipt/readback claims completion without fresh TaskProgressRead. Owner/account/Task-bound durable artifact lifecycle, one shared compound Draft/Attempt registration and capability exposure remain shared contract gaps |
