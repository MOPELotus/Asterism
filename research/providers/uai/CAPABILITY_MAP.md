...
| SubmissionVerify | UnipusHelperPro | Reference | Exact receipt-versioned fresh user-module readback independently binds Course/Group/version plus the complete ordered Question set, submitted states and answers. Session-aware verification first requires and decodes the same exact complete UAI media type/phase/nonzero-revision/digest manifest against the immutable Draft, so a foreign continuation fails before Task or user-module transport; media-free execution retains the legacy verifier. When the same bounded readback summary identifies donor-counted `questionType=1|3`, its `score_avg` becomes a validated fixed-point `0..=100` score; a real zero is preserved for policy/alerting rather than treated as absence. The ordinary verifier's Provider-private evidence owner additionally binds the exact result digest and preserves positive `strategyId`, required/recording flags, fixed-point `task_mini_score_pct`, availability window and submit state/timestamp. Complete legacy responses without strategy material remain compatible, while partial or malformed policy fails closed. Upload and compound adapters still project only their existing verification/score owners. The shared snapshot cannot carry those private policy facts, and score/answer persistence does not explain `pass/pass2/perm`, so UAI intentionally supplies no SubmissionVerify completion-diagnosis override. Uncounted or absent summaries yield no score, and missing receipts are Inconclusive without a guessed route |
