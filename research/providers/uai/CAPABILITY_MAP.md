...
| ResourceExecution / ExecutionVerify | UnipusAI + UnipusHelperPro + AutoFinish + Asterism Core | Reference | Five agreed pure-study base labels require a fresh exact `tab_type=text|video` progress leaf; exact `exit-ticket`, exact single `discussion`, and each exact single oral family require a fresh `tab_type=task` leaf. Every mutation also requires the fresh donor strategy window to be open. Pure-study execution freezes Master-resolved `marker|placeholder`: marker uses the MIT/current donor no-Question `submitType=2` body, while placeholder reproduces Apache's `instanceId=0` empty child/score-map and base-list padding/truncation into ordered judges with fresh Course version. Exit-ticket uses its exact marker. Direct discussion independently freezes the same two choices, with a homogeneous discussion placeholder. Oral uses its bounded placeholder-answer shape and fresh-version rule. Each mutates at most once and uses the Provider's fresh exact progress verify-only path without duration inference or POST replay |
...
18. freezes marker or Apache placeholder mode for `rich-text-read`, `text-learn`,
    ...
    placeholder mode for exact single discussion; and uses the donor's bounded
    placeholder-answer body for exact single oral families. The Provider returns an
    ...
    exit-ticket/discussion/oral and
