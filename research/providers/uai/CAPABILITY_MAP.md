...
| ResourceExecution / ExecutionVerify | UnipusAI + UnipusHelperPro + AutoFinish + Asterism Core | Reference | Five agreed pure-study base labels require a fresh exact `tab_type=text|video` progress leaf; exact `exit-ticket`, exact single `discussion`, and each exact single oral family require a fresh `tab_type=task` leaf. Every mutation also requires the fresh donor strategy window to be open. Preset and exit-ticket send the donor-observed empty `submitType=2` body. Direct discussion execution freezes Master-resolved `marker` or `placeholder` mode: marker uses the same no-Question body, while placeholder reproduces donor `instanceId=0` empty child/score-map/judge structure with the freshly rediscovered positive Course publish version. Oral uses its distinct bounded placeholder-answer shape with the same fresh-version rule. Each mutates at most once and uses the Provider's fresh exact progress verify-only path without duration inference or POST replay |
...
| Master runtime settings | Asterism Core + AutoPlayer | FromScratch | Versioned Provider schema v2 exposes conservative platform/account execution concurrency, account scan interval, and platform/account/Task overrides for bounded page-residence seconds, optional video playback and direct-discussion `marker|placeholder` mode; Core remains owner of resolution, scheduling and immutable execution snapshots |
...
18. submits the exact empty body for `rich-text-read`, `text-learn`,
    ...
    marker or placeholder mode, while exact single oral families use the
    donor's separate bounded placeholder-answer body; the Provider returns an
