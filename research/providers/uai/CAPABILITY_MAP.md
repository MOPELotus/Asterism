...
| File upload | AutoFinish | Reference | Provider-private native boundaries cover fresh exact Task-detail upload intent, CMS grant acquisition, bounded multipart object upload and the final single-`multiFileUpload` answer mutation. Task fingerprint, Course/Unit/Group, unique module position, object key, artifact digest and intent fingerprint remain bound across all stages; after object upload, a second fresh Task detail must match exactly and supply the current Course publish version. The final native POST rebinds Course instance and exact incomplete available Unit-progress leaf, uses donor `instanceId=0` with one key-valued completed child, returns only an accepted version receipt and never claims completion. Bounded zeroizing audio ownership and donor minimal-MP3 construction are covered. Owner/account/Task/Draft-bound durable artifact lifecycle, receipt-versioned upload readback and atomic mixed `multichoice,multiFileUpload` integration remain shared contract gaps |
