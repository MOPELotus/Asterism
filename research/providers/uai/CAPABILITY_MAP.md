...
| DurationRead / Task study record | UnipusHelperPro | Reference | Native user-info + per-Unit `unitTaskSituation` read binds CourseResource/Unit/Group exactly, and the returned Provider-private owner retains all three route components plus the complete normalized remote Task identity. Its snapshot preserves bounded Task `finishProgress`, donor-documented duration seconds, optional `required`, `scoreTaskFlag` and `taskQuesTotalScore`; shared DurationRead projects only seconds instead of discarding or conflating the other independent facts. Shared exposure for the richer Task study record remains a separate Core contract item |
...
| SubmissionVerify | UnipusHelperPro | Reference | Exact receipt-versioned fresh user-module readback independently binds Course/Group/version plus the complete ordered Question set, submitted states and answers. Session-aware verification first requires and decodes the same exact complete UAI media type/phase/nonzero-revision/digest manifest against the immutable Draft, so a foreign continuation fails before Task or user-module transport; media-free execution retains the legacy verifier. When the same bounded readback summary identifies donor-counted `questionType=1|3`, its `score_avg` becomes a validated fixed-point `0..=100` score; a real zero is preserved for policy/alerting rather than treated as absence. The ordinary, single-upload, compound-upload and compound-oral verifiers' Provider-private owners additionally bind the exact result digest after their adapter-specific exact readback succeeds; single and compound upload retain that digest directly even when optional policy fields are wholly absent. Positive `strategyId`, required/recording flags, fixed-point `task_mini_score_pct`, availability window and submit state/timestamp remain additional optional policy evidence. Complete legacy responses without strategy material remain compatible, while partial or malformed policy fails closed. The shared snapshot cannot carry those private policy facts, and score/answer persistence does not explain `pass/pass2/perm`, so UAI intentionally supplies no SubmissionVerify completion-diagnosis override. Uncounted or absent summaries yield no score, and missing receipts are Inconclusive without a guessed route |
| Answer Evidence Corpus / completion policy | Full four-donor sweep | Reference | UAI has no account-wide attempt/version enumeration route. The MIT user-module path is exact only with an accepted receipt version; the Rust `fetch_my_records` helper is unused, explicitly pre-reserved and proves neither current-version nor history semantics. Normal SubmissionVerify can incrementally contribute exact submitted child values and optional score, while `answerList.done/right/questionType/student_answer/versions` is evidenced but cannot be losslessly mapped through the current module-level Composite contract. Strict Completion uses only fresh `pass=pass2=perm=1`; score and Course/Unit finish progress remain independent. All execution donors skip completed Groups and expose no reset/redo/retake authority, so default-on Score Improvement must report UAI retake unavailable/unknown rather than resubmit a completed Task. Owner-global corpus, read-only bootstrap job, per-child evidence result, dual policy machines and scoring-policy reconciliation are Core/Domain gaps |
| ResourceExecution / ExecutionVerify | UnipusAI + UnipusHelperPro + AutoFinish + Asterism Core | Reference | Five agreed pure-study base labels require a fresh exact `tab_type=text|video` progress leaf; exact `exit-ticket`, exact single `discussion`, exact homogeneous `short_answer`, and each exact single oral family require a fresh `tab_type=task` leaf. Every mutation also requires the fresh donor strategy window to be open. Pure-study execution freezes Master-resolved `marker|placeholder`: marker uses the MIT/current donor no-Question `submitType=2` body, while placeholder reproduces Apache's `instanceId=0` empty child/score-map and base-list padding/truncation into ordered judges with fresh Course version. Exit-ticket uses its exact marker. Direct discussion independently freezes the same two choices, with a homogeneous discussion placeholder. Exact homogeneous `short_answer` independently exposes Apache's `SUBJECTIVE_ALLOW_EMPTY` placeholder execution while retaining its ordinary SubmissionExecute/SubmissionVerify path. Oral uses its bounded placeholder-answer shape and fresh-version rule. After every fresh preflight, all seven marker/placeholder families now pass their exact common POST URL, content type and complete zeroizing body through the same redacted request owner/digest boundary as ordinary answer submission. Each ResourceExecution mutates at most once and uses the Provider's fresh exact progress verify-only path without duration inference or POST replay. When that verification remains incomplete, only a fresh positive Group close time at or before the progress observation maps to shared `WindowClosed`; all individual completion-flag, score, duration, required-child, prerequisite and review explanations remain `RemoteUnknown` because donors do not define those mappings. Persisting the exact mutation digest before TaskExecution dispatch remains a shared Core contract gap |
| DurationReport | AutoPlayer | Reference | Active first-batch BrowserBridge path: real rendered Unit/Section/Micro → Tab → Task traversal, bounded page residence, popup handling and optional video playback/keepalive followed by fresh DurationRead. The donor's Course-level `menuList.slice(startIdx)` semantics now have a Provider-local immutable `CourseResidenceBatchPlan`: fresh Task inventory preserves source-tree order instead of remote-ID sorting; the plan freezes complete ordered Micro/Group membership, Course publish version, total budget, video setting and identity-bound start/restart target. A separate bounded credential-free child value freezes provider/batch versions, explicit runtime-profile digest, owner Task, complete ordered Task fingerprints and the exact start authority, then reconstructs the batch only from fresh Course/Task/settings inputs. Its namespaced Core artifact is a compact JSON projection with exact identity/start/batch facts plus ordered-task and full-child digests; even the 8192-Task maximum remains below Core's 64 KiB artifact limit because recovery freshly rebuilds the full list rather than deserializing a truncated copy. Its rational Micro share delays positive `Math.round` until runtime Tab/Task cardinality is known, matching donor distribution without early floor loss. Shared planning attachment/browser execution remains a Core Gap, not a deferral |
...
| Protocol drift observation | Asterism Core + UAI fail-closed parsers | FromScratch | Three high-value `ProtocolDrift` exits attach bounded sanitized shape only: unknown Task-tree roles record a role SHA-256/byte length plus node field count and child presence; unknown `video-popup` module/child reply types record scope, JSON kind and, only for strings, SHA-256/byte length; malformed or oversized score summaries record only summary/`answerList` JSON kinds, presence and collection cardinality. No raw role/reply value, response, answer, field-name list, route, identity or credential material is retained. Unsupported composite-child kinds, incompatible mixed child shapes, individual score-row `questionType` drift and generic field drift remain fail-closed without an observation until an equally minimal schema is justified. |
| BrowserBridge | AutoPlayer | Reference | Provider fresh-rediscovers the exact Group and returns a hashed account/Task isolation key plus exact `ucontent`/`ipub` origins in non-headless mode. Its private plan derives the secret-free HTTPS start route `/_explorationpc_default/pc.html?cid={fresh CourseResource}` after checking Course/Group identity consistency, then rebinds normalized Unit/Section/Micro/Task labels. Plan v2 freezes legacy/Ant/ARIA/u3menu discovery, the donor's ordered four-selector iframe lookup, exact Tab/Task/popup/video selector sets, the 30-second MutationObserver plus 20×1.5-second iframe scan window, DOM/action/cardinality/time bounds, master-resolved residence/video settings and mandatory session-nonce+frame+exact-origin messaging; validation compares every selector and timing field exactly rather than merely requiring non-empty lists. Source re-audit confirms wildcard `SCAN`/`CLICK`/`PING` is the `ipub` iframe compatibility channel, while the top page directly owns menu/Tab/Task/residence/video actions. Asterism maps both sides to typed correlated commands/events: transport-observed origin must equal the envelope origin, nonces are zeroized/redacted, discovered labels are bounded, and clicks accept only a unique fresh hierarchy match through an opaque session-bound handle rather than arbitrary selectors/DOM paths. The broad final donor iframe fallback is usable only when the selected frame independently observes exact `ipub` origin and session frame binding. Bounded ordered Tab/Task snapshots derive their handles from the same binding; only one exact target Task wrapper can construct the final residence action. The result parser bounds Micro/Tab/Task counts and timing, rejects foreign command or unexpected video, and still requires fresh DurationRead. Stable message types and SHA-256 helpers feed Core's durable exchange ledger without Provider-private JSON. Issuance returns typed command, encrypted artifact and matching exchange as one owner; recovery uses persisted session/sequence/digest plus artifact, never helper command echo. Event/residence completion fresh-rebind Task/settings, compare transport origin and attach exact raw document digest. A separately issued pause/resume/restart command can also recover its exact command artifact and produce a typed redacted control acknowledgement plus completed exchange; command derivation from an active `ResidenceTarget` copies its exact session/frame/Task handle and rational leaf seconds instead of widening back to the Course budget, while deliberately granting no cursor mutation or active-exchange cancellation. A separate immutable Course batch plan supplies the donor-complete ordered Micro workload and identity-bound restart authority. Its version-4 accumulated cursor has an independent bounded zeroizing artifact and stable digest, rejects unknown schema fields and rebinds the exact fresh batch plan, Browser plan and next command after recovery. Cursor-aware issuance now consumes both artifacts into Core's atomic encrypted runtime-state sidecar under stable type `uai.browser.cursor.v4`; strict consuming recovery checks command/state digests and exact session/sequence/issued-at metadata before fresh command-first/cursor-second rebinding, while helper dispatch receives only the command. Terminal residence remains verify-only, but its exact Task/completion/result identity can consume an independent fresh study-record read and produce a batch/Browser-plan-bound duration readback owner without inventing a success delta. Session injection, actual DOM action dispatch, full Course batch ownership/orchestration and active control ownership continue through shared executor gaps |
...
| Discussion | AutoFinish | Reference | Exact single-discussion Tasks expose both donor direct-empty ResourceExecution shapes independently from the richer reply flow: a frozen runtime choice selects `submitType=2` no-Question marker or `submitType=1` `instanceId=0` placeholder with fresh Course judge version. Both fresh-rebind the exact task leaf, mutate once and require fresh progress verification. Provider-private reply boundaries additionally cover fresh Course/class/curricula/current-user binding, exact topic lookup, bounded route-topic-bound reply pagination, zeroizing immutable reply intent, reply mutation receipt and exact current-user/content readback. The first mutation exposes a stable request digest over all route/account/topic/content facts for pre-dispatch registration. Only exact reply readback plus one fresh single-discussion Task can freeze the separate Group-completion mutation, whose digest now also binds the original reply request before sending the empty marker body. A credential/content-free `uai.discussion.reply-plan.v1` artifact projects two independent single-occurrence `AcceptedMaximumReached` phases; phase two requires a `uai.discussion.reply-readback.v1` observation whose digest joins the original reply intent, verified reply and exact dynamic completion request. Both definite accepted response owners retain separate response digests, while rejected/malformed/ambiguous results create no receipt. Core recovery snapshots are revalidated into read-only no-evidence/ambiguous/readback-gate/awaiting-progress states and never authorize replay. Fresh TaskProgressRead remains final. Cross-Provider execution still lacks the scheduling-time immutable discussion Draft/capability that owns zeroizing reply content and invokes this sequence |
| Exit-ticket / oral execution | AutoFinish | Reference | Exit-ticket has a dedicated exact-task empty completion path. Exact single `oral-sentence`, `video-dub` and `oral-personal-state` Groups use the donor's separate bounded placeholder-answer body. The audited ordered `basic-scoop-content,oral-sentence` Group has a Provider-private atomic plan: it joins one immutable matching sub-Draft with the same Group's fresh encrypted oral instance and bounded per-child value/`answersExtra` evidence, maps `answersExtra` to the submitted child `extra`, submits both modules once, and verifies every value/extra through receipt-authorized user-module readback. After fresh route/account rebinding, its zeroizing request owner exposes a stable digest over exact POST URL, content type and complete body while redacting all wire material; this request identity stays distinct from the semantic plan fingerprint. Empty scalar/array and donor-stable non-empty text/list/number/bool judge shapes are covered; object values without a stable donor judge encoding fail closed. All paths require fresh task-leaf preflight, accepted version receipts and fresh progress verification with no ambiguous replay. This is one remote mutation, so the new fixed dependency DAG adds no missing Provider semantics; shared compound Draft/artifact/capability exposure remains the blocker |
| File upload | AutoFinish | Reference | Provider-private native boundaries cover fresh exact Task-detail upload intent, CMS grant acquisition, bounded multipart object upload, final single-`multiFileUpload` mutation and receipt-versioned user-module readback. Task-tree parsing preserves that donor-canonical camel-case label instead of lowercasing it past the upload gates. Task fingerprint, Course/Unit/Group, unique module position, object key, artifact digest and intent fingerprint remain bound across all stages. Each freshly materialized HTTP stage now owns its exact pre-dispatch digest: CMS grant binds GET plus account/Course query and Referer, Qiniu binds POST/origin/content-type plus the complete token/key/artifact multipart body, and both single and compound final submit bind POST URL/content type plus the complete zeroizing account/Course/body document. Semantic fingerprints remain separate from these network identities. The mixed path atomically joins one validated ordinary sub-draft plus the uploaded artifact and verifies both ordered values. Neither receipt/readback claims completion without fresh TaskProgressRead. Core's receipt-conditional atomic sequence represents the bounded stage-advance rules without relabeling the Qiniu receipt as independent verification. For an already prepared single/compound final plan, a redacted scheduling artifact and request-level semantic digest bind one issue-time dynamic `uai.upload.final-submit` phase with a two-attempt ceiling and `AcceptedOrMaximumReached`; the compound semantic digest directly includes ordinary protocol versions, ordered Question identity/type, every ordered child value and every judge type in addition to the immutable Draft reference. First success short-circuits, only exact numeric `600001/600002` rejection creates a retryable `accepted=false` receipt with 120 seconds, and Core persists its `retry_not_before` and rejects ordinal two until the deadline; ambiguity and every other rejection record no receipt. UAI's bounded zeroizing `uai.upload.final-plan-state.v1` codec now owns the exact single key or compound Draft/answer/judge material plus the materialized final request digest and decodes only when it reproduces the independently persisted compact artifact and sequence digests. A definite version-bearing acceptance freezes encrypted `uai.upload.final-result.v1` bytes binding kind, ordinal, original accepted timestamp, sequence plan/artifact, semantic plan, request/response digests and submission version; decode requires the exact accepted Core issue/receipt lineage. Combining both exact states requires identical request digests, reconstructs only the accepted receipt, parses exact key/answer readback and returns a same-ordinal `ExecutionMutationVerification` over the readback digest. A foreign object key, same-Draft-ID selected-answer substitution or another Course-instance/account request fails before evidence exists. The full flow is not yet executable because a hash-only Qiniu receipt cannot restore the returned key, Core has no owner/account/Task/Execution/Attempt-bound encrypted stage repository for the input/grant/object/final-plan/final-result chain, and it cannot atomically persist the relevant mutation receipt with its successor state and later verification. The generic parent-authority/batch snapshot is not this receipt-conditional stage transaction. Retry wake-up/executor wiring, immutable single/compound Draft ownership and shared capability exposure remain Core gaps |
| External answer / media transcription | AutoFinish + current Rust donor | Reference | LLM answer generation and audio/video transcription are donor capabilities; UAI supplies bounded opaque audio/video/subtitle attachments, embedded transcript semantics and exact HTTPS media sources without placing fetch URLs in Domain Questions. Core now persists the optional complete-set artifact atomically with the immutable snapshot and passes its claimed decrypted value only to session-aware AnswerResolve. UAI recomputes every attachment ID and rejects digest, Task, Question, URL, kind, order, manifest and schema drift before exposing a bound source. From that rebound manifest it can freeze one exact GET authority whose digest covers Task/Question/fingerprint/attachment/URL/kind, assigns bounded subtitle/audio/video response ceilings, permits the captured session only for the standard `https://ucontent.unipus.cn` origin, keeps external or custom-port routes anonymous and rejects redirects/final-route drift. A complete per-Question batch also preserves every donor-order request and hashes their exact ordered request digests; duplicates, foreign/media-free Questions and reordering fail closed without adding retry/skip policy. Its zeroizing response owner then accepts only status 200, the same final route and nonempty in-bound bytes; a separate response digest binds the request digest, exact byte digest and length without inventing MIME/signature rules absent from donor evidence. When every request succeeds, a complete response-set owner rechecks exact count/order/request digests and hashes the batch plus ordered response digests; missing, duplicate or reordered results cannot masquerade as the batch. A bound downloaded VTT/SRT response can enter the donor-evidenced bounded header/ordinal/timestamp/cue normalizer and yields a zeroizing transcript whose digest also binds the response. Invalid UTF-8, empty/oversized text and non-subtitle responses fail closed. A safe downloader still requires shared DNS/private-range resolution and bounded streaming execution; audio/video model credential, transcription, partial-result policy and external answer orchestration remain Core gaps |

Compound-oral verification now retains its exact validated readback digest
...
response. Native upload retains both the exact multipart request digest and
response digest in the uploaded-artifact owner. Bounded zeroizing
`uai.upload.object.v1` bytes reconstruct that complete successor only against
the independently persisted issue/receipt digests; the grant token is not
copied forward. This closes Provider output recovery but not Core's atomic
