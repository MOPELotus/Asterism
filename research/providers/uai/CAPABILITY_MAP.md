...
| External answer / media transcription | AutoFinish + current Rust donor | Reference | LLM answer generation and audio/video transcription are donor capabilities; UAI supplies bounded opaque audio/video/subtitle attachments, embedded transcript semantics and ephemeral exact HTTPS media sources without persisting fetch URLs. Every source owner repeats the exact Task and remote Question, and its opaque attachment digest includes both identities plus the canonical URL, preventing cross-Task reuse of a shared CDN URL. A safe downloader still requires shared DNS/private-range validation and host-scoped authentication; donor behavior forwards default credentials to arbitrary media hosts and is not copied. Model credential/external AnswerResolve orchestration remains a Core contract gap |
