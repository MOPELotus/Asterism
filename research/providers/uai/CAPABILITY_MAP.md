...
| SubmissionVerify | UnipusHelperPro | Reference | Exact receipt-versioned fresh user-module readback independently binds Course/Group/version plus the complete ordered Question set, submitted states and answers. When the same bounded readback summary identifies donor-counted `questionType=1|3`, its `score_avg` becomes a validated fixed-point `0..=100` score; a real zero is preserved for policy/alerting rather than treated as absence. Uncounted or absent summaries yield no score, and missing receipts are Inconclusive without a guessed route |
| ResourceExecution / ExecutionVerify | UnipusAI + UnipusHelperPro + AutoFinish + Asterism Core | Reference | Five agreed pure-study base labels require a fresh exact `tab_type=text|video` progress leaf; exact `exit-ticket` and each exact single oral family require a fresh `tab_type=task` leaf. Every mutation also requires the fresh donor strategy window to be open. Preset/exit-ticket send the donor-observed empty `submitType=2` body, while oral sends its distinct bounded `instanceId=0` placeholder-answer shape and now requires/injects the freshly rediscovered positive Course publish version into every judge instead of retaining the donor's captured example constant. Each mutates at most once and uses the Provider's fresh exact progress verify-only path without duration inference or POST replay |
...
| Discussion | AutoFinish | Reference | Provider-private native boundaries cover fresh Course/class/curricula/current-user binding, exact topic lookup, bounded route-topic-bound reply pagination, zeroizing immutable reply intent, reply mutation receipt and exact current-user/content readback. The first mutation now exposes a stable request digest over all route/account/topic/content facts for pre-dispatch registration. Only exact readback plus one fresh single-discussion Task can freeze the second mutation, whose separate request digest binds Task fingerprint/hierarchy/topic and verified reply digest before native completion rechecks progress and sends one empty `submitType=2` POST. Reply and Group receipts remain independent and neither claims completion; fresh TaskProgressRead remains final. Cross-Provider registration and durable recovery still require the shared immutable discussion Draft/attempt contract |
| Exit-ticket / oral empty execution | AutoFinish | Reference | Exit-ticket has a dedicated exact-task empty completion path. Exact `oral-sentence`, `video-dub` and `oral-personal-state` Groups use the donor's separate bounded placeholder-answer body. The audited ordered `basic-scoop-content,oral-sentence` Group also has a Provider-private atomic plan: it joins one immutable matching sub-Draft with the same Group's fresh encrypted oral instance/empty-child evidence, submits both modules once, and verifies both through the receipt-authorized user-module readback. All paths require fresh task-leaf preflight, accepted version receipts and fresh progress verification with no ambiguous replay. Shared compound Draft/Attempt exposure remains a Core Gap; non-empty oral material and dynamic `answersExtra` remain fixture-gated rather than guessed |
| File upload | AutoFinish | Reference | Provider-private native boundaries cover fresh exact Task-detail upload intent, CMS grant acquisition, bounded multipart object upload, final single-`multiFileUpload` mutation and receipt-versioned user-module readback. Task-tree parsing preserves that donor-canonical camel-case label instead of lowercasing it past the upload gates. Task fingerprint, Course/Unit/Group, unique module position, object key, artifact digest and intent fingerprint remain bound across all stages. The deterministic Qiniu request additionally exposes an exact digest over fixed method/origin/content-type and the complete token/key/artifact multipart body for pre-dispatch registration. Grant and final-submit exact request digests must be generated later at their fresh account/Course-instance send boundaries rather than treating semantic fingerprints as HTTP identities. The mixed path atomically joins one validated ordinary sub-draft plus the uploaded artifact and verifies both ordered values. Neither receipt/readback claims completion without fresh TaskProgressRead. Durable artifact lifecycle, one shared compound Draft/Attempt and capability exposure remain Core gaps |
| External answer / media transcription | AutoFinish + current Rust donor | Reference | LLM answer generation and audio/video transcription are donor capabilities; UAI supplies bounded opaque audio/video/subtitle attachments, embedded transcript semantics and exact HTTPS media sources without placing fetch URLs in Domain Questions. The Provider-specific parser now returns the Question and optional encoded artifact together; the routes cross the durable QuestionSession boundary only as a bounded encrypted artifact whose digest binds the Core Task, remote Group/Question, position, Question content fingerprint, exact ordered attachment IDs and canonical URLs. Decode recomputes every attachment ID and rejects session, Question, URL, kind, order and schema drift. Shared Provider API/Engine registration must still select this richer adapter instead of discarding the optional artifact. A safe downloader still requires shared DNS/private-range and redirect validation plus host-scoped authentication; model credential/external AnswerResolve orchestration remains a Core gap |
