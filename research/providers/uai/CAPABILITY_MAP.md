...
| External answer / media transcription | AutoFinish + current Rust donor | Reference | LLM answer generation and audio/video transcription are donor capabilities; UAI supplies bounded opaque audio/video/subtitle attachments, embedded transcript semantics and exact HTTPS media sources without placing fetch URLs in Domain Questions. The routes can now cross the durable QuestionSession boundary only as a bounded encrypted Provider artifact whose digest binds the Core Task, remote Group/Question, position, Question content fingerprint, exact ordered attachment IDs and canonical URLs. Decode recomputes every attachment ID and rejects session, Question, URL, kind, order and schema drift. A safe downloader still requires shared DNS/private-range and redirect validation plus host-scoped authentication; donor behavior forwards default credentials to arbitrary media hosts and is not copied. Model credential/external AnswerResolve orchestration remains a Core contract gap |
