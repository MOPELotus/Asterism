...
| Coverage is computed from selected/supported Questions only | Samueli defaults `cover_rate` to 0.9 and OCS divides resolved Questions by all Questions | Bind the complete fresh snapshot denominator, threshold and exact unanswered set into the Draft; unknown/unsupported types lower coverage and never receive invented values |
