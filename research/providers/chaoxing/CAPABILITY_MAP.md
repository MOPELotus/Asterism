...
| Sign-in QR material binding | `Ylim314/chaoxing-sign` | `mini-hbut` QR scanner | Reference | Provider-local, unregistered parser accepts only an exact freshly rebound QR activity plus an allowlisted HTTPS QR URL or `SIGNIN:` payload. It requires one matching activity ID, one bounded `enc`, exact `source=15` for `SIGNIN:` and an optional refresh code, then retains only redacted zeroizing material plus binding/material digests. Duplicate aliases, foreign hosts/activities and non-QR variants fail closed. This is Capture input preparation only: it exposes no plaintext material, dispatch, Receipt, retry or completion claim |
...
- The ordinary-sign preparation accepts only the exact list/detail discovery
  pair plus bounded UID/FID/name, freezes separate zeroizing `preSign` and
  `stuSignajax` query templates, and exposes only static route names, the
  two-request count and an actor-bound SHA-256 digest. It has no transport or
...
- A separate QR-material boundary now accepts one scanned allowlisted HTTPS URL
  ...
  does not resolve the conflicting donor `preSign` sequence.
