...
- A separate QR-material boundary now accepts one scanned allowlisted HTTPS URL
  ...
  `source=15` and optional refresh code into zeroizing redacted state with
  separate binding/material digests. It has no query getter or send method and
  does not resolve the conflicting donor `preSign` sequence. From that exact
