...
| Sign-in activity read / preparation / receipt | `Ylim314/chaoxing-sign` | `mini-hbut`, Samueli | PortSource / Reference | Unregistered Provider-local Native GET boundary for exact course-bound `activelist` and activity-bound `signDetail`; bounded/zeroizing documents, numeric-or-string identities, nested/direct time shapes, duplicate rejection and fresh identity/type/variant/time rebinding are offline-covered. A separate immutable ordinary-sign preparation explicitly freezes the exact Ylim `preSign -> stuSignajax` family and actor-bound digest without exposing a send method; Samueli and `mini-hbut` corroborate overlapping fields but use different complete parameter/route chains and are never spliced into it. A donor-specific auxiliary boundary separately freezes Ylim's exact `analysis` request, extracts one bounded dynamic code from its bound response and freezes `analysis2`; it does not claim that the donor's location-labeled auxiliary pair is universally required for ordinary sign-in. Bound pure parsers classify only an empty/exact-completed `#statuscontent` and exact `success`/already-signed/expired response strings as preflight evidence or Receipts. The donor states that issuing `preSign` is required for the later sign record, so its GET shape is a mutation-sequence step rather than an independently safe read. Conflicting donor meanings for `status=1` and `otherId=5` remain raw/ambiguous; no read, preparation or Receipt proves independent account completion |
| Sign-in QR material / submit-step preparation | `Ylim314/chaoxing-sign` | `mini-hbut` QR scanner | Reference | Provider-local, unregistered parser accepts only an exact freshly rebound QR activity plus an allowlisted HTTPS QR URL or Ylim `SIGNIN:` payload. It requires exactly one activity alias across `activeId|id|aid`, one bounded `enc`, and carrier-specific fields: HTTPS permits only `activeId|id`, while `SIGNIN:` requires `aid`, exact `source=15` and a bounded `Code`. Cross-carrier aliases/fields fail closed. Material remains redacted and zeroizing with binding/material digests. A second immutable value freezes Ylim's exact QR `stuSignajax` field set against that material and explicit actor identity, exposes only the static route/count/digests and has no plaintext query or send method. Foreign hosts/activities, stale material and non-QR variants fail closed. The donor-conflicted `preSign` step, durable sequence authority, Receipt and independent completion readback remain absent |
...
| QuestionInventory / QuestionParse | `Samueli924/chaoxing`, OCS current preview pages | CxKitty, `chaoxing-exam` | PortSource / Reference | Independent Work and Chapter Work have offline-covered fresh-page reads with account/correlation/task-bound attempt caches; Chapter Work rebinds all seven cards and its ephemeral `jobid`/`enc`/`ktoken` before one non-replayed attempt GET; pending Exam tasks use cover -> one-shot start -> full attempt-bound mobile preview and retain only the rotated bounded attempt state. Cover `testUserRelationId` is a unique raw attempt identity; the start page may omit it and reuse the already bound value, but if present it must also be unique and exact. Start `enc` is unique across URL/HTML, both copies must agree, and timing inputs are unique untrimmed decimals. The CxKitty-observed `var *needcode *= *(\d+);` cover flag must appear exactly once: 0 is false, any bounded nonzero integer is true, and missing/duplicate/malformed/lookalike declarations are ProtocolDrift. Observed `faceRecognitionCompare` and `captchaCheck` inputs are each required exactly once with raw value `0|1`; missing, duplicate, padded or other values are drift. Exam-code/face/captcha gates return typed BrowserRequired |
...
| SubmissionBuild / Execute | `Samueli924/chaoxing` | CxKitty, OCS, agent skill | Reference | Independent Work, Chapter Work and Exam execute Core-frozen partial coverage without inventing answers. Work forms preserve the complete current control order and rebuild unanswered values empty. Independent Work, Chapter Work and Exam accept a parsed `blank_count=1` plus exactly one Draft text; ambiguous multi-blank concatenation fails closed. Independent Work still rejects short-answer mutation because its exact result/grading readback is not established. Exam's encrypted v3 continuation retains the complete ordered Question identity/fingerprint set plus the frozen selected original positions; saves traverse only selected Draft Questions and send each donor `start` as its original zero-based paper index. No Exam mutation is routed through Work payloads or replayed after ambiguity |
| SubmissionVerify | agent skill, `Samueli924/chaoxing` | `chaoxing-exam`, OCS | PortSource / Reference | Independent Work verifies exact server-visible answers: each selected result node must contain exactly one `我的答案` label, choice values allow only uppercase option IDs plus explicit separators, duplicates and single-choice multi-values are drift, and true/false accepts only complete controlled tokens rather than substrings. Controlled empty/pending placeholders yield Inconclusive with all Draft Questions Unverified, not ProtocolDrift or rejection. Chapter and Exam result identity attributes are validated without trimming; padded QIDs are protocol drift rather than rebound identities. Exam applies the same unique-label rule before its own exact value parser. Chapter Work refreshes seven cards and has strict current verification / standard-answer parsers awaiting its BrowserBridge iframe route; Exam uses Completed only for task recovery, requires the full result Question count and unique identities, and confirms/rejects only selected Draft Questions at their original positions from exact type/value evidence. Exact single-blank submitted text is supported only when the Question binds one blank; pending placeholders are not evidence. Unselected or unsupported result Questions are not answer evidence. The independent bounded result score remains fixed thousandths. A fresh explicit Expired state maps to `WindowClosed`; all less specific incomplete states remain conservatively undiagnosed |
...
- The ordinary-sign preparation accepts only the exact list/detail discovery
  pair plus bounded UID/FID/name, explicitly identifies
  `YlimPreSignStuSignAjax`, freezes that donor's separate zeroizing `preSign`
  and `stuSignajax` query templates, and exposes only static route names, the
  two-request count and an actor-bound SHA-256 digest. Ylim's exact normal
  pre-sign fields do not include Samueli/mini's `tid` or `ut`; the previous
  hybrid template has been removed. It has no transport or plaintext query
  getter. Photo, location, gesture, QR and conflicted/unknown variants fail
  before preparation because their upload/input/prerequisite semantics are not
...
- A separate Ylim-specific auxiliary preparation records the exact unconditional
  ...
  into the exact Ylim ordinary pair. It freezes `pptSign/analysis` with
...
- A separate QR-material boundary now accepts one scanned allowlisted HTTPS URL
  ...
  `QrCode`. It requires exactly one ID alias across all recognized carriers:
  HTTPS accepts `activeId|id` and rejects `aid`/`Code`/`source`, while Ylim
  `SIGNIN:` accepts only `aid` plus bounded dynamic `enc`, exact `source=15` and
  required bounded `Code`. The material is zeroizing/redacted with separate
  binding/material digests. It has no query getter or send method and does not
  resolve the conflicting donor `preSign` sequence. From that exact
  material, a second preparation freezes Ylim's actor-bound QR `stuSignajax`
  fields and request digest, but still exposes no query or transport and cannot
  stand alone as a complete mutation sequence.
...
- The same fresh pending independent Work state now advertises
  ...
  the native subset. Build remains value-free. Execute accepts single-choice,
  multiple-choice, true/false and exactly one page-bound fill blank with
  unambiguous donor encodings, rechecks the complete immutable Draft and fresh Work route,
  ...
  Question. Every selected Work/Exam result node must contain exactly one
  `我的答案` label; a second label is structural drift rather than a delimiter.
  Choice parsing rejects arbitrary surrounding text, duplicates and
  multi-valued single-choice results instead of filtering/deduplicating them;
  true/false requires one complete controlled token, so values such as
  `不正确` or `错误但对` cannot be promoted by substring matching. Exact
  `未作答`/`待批阅`/`暂无答案`-family placeholders are missing evidence and make
  the snapshot Inconclusive with Draft Questions Unverified. Multi-blank and
  short-answer Work mutations remain fail-closed. This is synthetic/offline
  evidence only and does not raise the Provider above Development.
- Core-frozen partial coverage is now executable for independent Work, Chapter
  ...
  answer evidence only for selected Questions. Chapter `data` and Exam
  `questionId.value` identities must satisfy the QID grammar as stored in the
  DOM; leading/trailing whitespace is not trimmed into a valid binding.
...
- Pending independent Exam rows with a structural `goTest` entry now advertise
  ...
  errors remain locked rather than replayed. The cover parser requires one exact
  decimal `needcode` declaration matching the pinned CxKitty grammar; it never
  defaults a missing declaration to false or tests only for digit `1`. The
  observed face/captcha hidden inputs are likewise unique required binary
  fields and are not defaulted false when absent. Exam-code, face and captcha
  gates return typed `HumanRequired` pending their BrowserBridge/Capture command
  path. The cover attempt ID and any present start-page attempt ID are read from
  one unique raw `testUserRelationId` input; whitespace is not trimmed, and a
  duplicate is never resolved by taking the first element. Dynamic `enc` is
