...
| CxKitty Exam temporary save/final submit and code/face/captcha gates | durable Exam attempt/save/final operations plus HumanRequired | Native subset implemented; cover/start `testUserRelationId` is unique/raw, URL/form `enc` must be unique and agree, timing inputs are unique raw decimals, decimal `needcode` is parsed from one exact declaration, and face/captcha inputs must each be unique raw `0|1`; missing/malformed facts fail closed while browser challenge paths and the coupled persisted IMEI/mobile-User-Agent device profile remain pending |
...
|---|---|---|
| `0` single, `1` multiple, `3` true/false | all current donors | Parsed, native mutation and strict visible-answer verification. Selected Work/Exam result nodes require one unique `我的答案` label; Work choices accept only uppercase option IDs plus audited separators, reject duplicates/single-choice multi-values, and use complete true/false tokens rather than substring matching. Controlled pending/empty Work placeholders produce Inconclusive/Unverified rather than drift or rejection |
