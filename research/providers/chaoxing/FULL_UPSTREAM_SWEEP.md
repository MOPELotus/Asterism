...
| CxKitty Exam temporary save/final submit and code/face/captcha gates | durable Exam attempt/save/final operations plus HumanRequired | Native subset implemented; decimal `needcode` is parsed from one exact declaration with missing/malformed drift fail-closed, while browser challenge paths remain pending |
