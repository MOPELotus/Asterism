...
| CxKitty Exam temporary save/final submit and code/face/captcha gates | durable Exam attempt/save/final operations plus HumanRequired | Native subset implemented; decimal `needcode` is parsed from one exact declaration, and observed face/captcha inputs must each be unique raw `0|1`; missing/malformed gates fail closed while browser challenge paths remain pending |
