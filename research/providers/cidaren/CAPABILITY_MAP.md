...
5. **Pre-Question child prerequisite (`CIDAREN-CORE-GAP-QUESTION-BLOCKED`).**
   ...
   Cidaren now strictly recognizes only that exact envelope, preserves its
   request/response digest and receive time in a Provider-private definite
   rejection, and can encode them with both Task identities, current position
   and dynamic allocation as the bounded `cidaren.question-blocked-step.v1`
   recovery artifact. Provider recovery fresh-rebinds the Task/allocation and
   reconstructs only FailedClosed state. A single Provider materialization
   also freezes artifact + diagnosis + operation + both digests + receive time.
   The shared Question
   read outcome still cannot durably store that artifact plus
   `RequiredChildrenPending` without
   misclassifying the Task or authorizing a replay. Main must add a definite
   blocked remote-step outcome before the adapter can return this evidence as
   a successful durable step result.
