...
5. **Pre-Question child prerequisite (`CIDAREN-CORE-GAP-QUESTION-BLOCKED`).**
   ...
   Cidaren now strictly recognizes only that exact envelope, preserves its raw
   response digest/time in a Provider-private definite rejection and exposes a
   stable non-retryable provider code. The shared Question read outcome still
   cannot durably store those facts plus `RequiredChildrenPending` without
