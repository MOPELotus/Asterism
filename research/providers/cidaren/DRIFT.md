...
| `SubmitChoseWord` is parsed as a Question response or loose `20001` success | Donor validates it but does not decode a next step; shared handler requires truthy `data` for `20001` and accepts terminal `20004` | Use the separate acknowledgement-only parser, reject false/empty `20001`, terminate on `20004`, and require decoded data for Start/Verify/advance |
...
| Mutation success text overrides a failure code or is accepted as verification | Donor interprets localized strings only inside a successful response and later stops on them | Require the donor numeric/data success condition before classifying text, then re-list exact release identity and verify fresh progress/state after mutation |
