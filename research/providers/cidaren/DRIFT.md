...
| Pre-Question recovery assumes Start always yields a Question | Eligible tasks may require `SubmitChoseWord`, and mode 0 yields a reading card before any real Question | Rotate an encrypted `cidaren.pre-question-attempt.v1` continuation through selection/start/reading-card phases; materialize a QuestionSession only after a real Question exists |
...
| `topic_mode=73` is treated as an ordinary one-text completion | Public issue 99 shows two answers/two word lengths/no options and the donor explicitly reports it unimplemented | Parse and expose a bounded FillBlank Question, permit the independently evidenced Skip path, but reject AnswerResolve/Build/Verify until a live or upstream trace establishes the multi-answer encoding |
