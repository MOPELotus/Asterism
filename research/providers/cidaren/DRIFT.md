...
| A recovered Capture result is rebound with caller-selected metadata | Core durably receives encrypted raw result bytes before Provider parsing | Resolve the persisted metadata and `SecretValue` together; recheck result type/session/sequence/raw digest/receive time against the issued command, and derive terminal completion time only from that receipt |
