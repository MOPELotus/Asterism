...
| BrowserBridge opens an origin-only or generic page | Both audited Capture recipes and the native student Referer use `/student/` | Freeze policy revision 2 to `https://app.vocabgo.com/student/`, require its exact HTTPS origin in `allowed_origins` and recheck the start URL before command issue |
