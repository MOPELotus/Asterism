...
| `StartAnswer` query | Reopened UI fixes class request `task_type_int=2` (including selected learning rows), binds class `release_id` and passes literal `%23000000` as `opt_font_c`; decoded payload echoes row type 1/2 | Same current class query literals and response echo are retained by the owner line | Not relevant | Keep request selector (`Class=2`, historical `Study=3`) separate from response-row identity (`Class learning=1`, `Class test=2`, `Study=3`); standard query encoding must yield donor wire `%2523000000`, and a mismatched response echo fails after fresh binding |
...
| `SubmitChoseWord` | Yes; every eligible path submits a grouped `course_id:list_id -> words` map, and the current ordinary-learning path continues to `StartAnswer` when selection reports the exact already-started issues 48/49 envelope | Yes; same grouped map, but no continue-on-error call site | No | Implemented grouped one-shot transport/state edge; only the public donor's exact ordinary class rejection rotates to durable `ready-to-start`, preserving response binding and prohibiting another selection. Other routes and near matches fail closed |
