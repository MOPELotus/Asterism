...
| `SubmitChoseWord` | Yes; issues 48/49 add one exact incomplete-child rejection envelope | Yes | No | Implemented one-shot transport/state edge plus Provider-private definite `RequiredChildrenPending` classification with digest/time retention; shared durable blocked outcome remains a Core Gap |
