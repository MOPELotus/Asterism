...
| `SubmitChoseWord` | Yes; every eligible path submits a grouped `course_id:list_id -> words` map, and issues 48/49 add one exact incomplete-child rejection envelope | Yes; same grouped map | No | Implemented grouped one-shot transport/state edge plus Provider-private definite classification, exact recovery artifact/FailedClosed restore and atomic materialization of diagnosis + artifact + operation ledger facts; shared durable blocked outcome remains a Core Gap |
