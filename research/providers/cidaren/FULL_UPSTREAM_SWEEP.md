...
| `SubmitChoseWord` | durable grouped-word-map one-shot pre-Question mutation plus exact issues 48/49 prerequisite rejection classification | Grouped `course_id:list_id -> words` maps now match every eligible donor call site; Provider-private definite rejection, exact FailedClosed recovery and one atomic diagnosis/artifact/ledger materialization remain implemented, while the durable shared `RequiredChildrenPending` step remains a Core Gap |
