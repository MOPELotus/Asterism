...
| `SubmitChoseWord` | durable one-shot pre-Question mutation plus exact issues 48/49 prerequisite rejection classification | Implemented through Provider-private definite rejection, exact FailedClosed recovery and one atomic diagnosis/artifact/ledger materialization; durable shared `RequiredChildrenPending` step remains a Core Gap |
