...
| TaskDetail | Fresh Course/Unit/SCO inventory | FromScratch | Re-lists Courses and re-runs one complete Course scan, then requires exactly one matching stable SCO identity and one identical normalized Task snapshot in both public and nested detail views; mixed injected/restored snapshots fail closed before execution; live pending |
