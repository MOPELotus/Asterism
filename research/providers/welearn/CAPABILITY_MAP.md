...
persists the parent/settings/sequence facts and creates Requested child
Execution drafts. It must still activate/schedule those drafts and register the
composite dispatch entry before production execution can call the coordinator
...
private snapshot whose authority/batch digests it carries. Core `53859f8` now
provides `ProviderBatchExecutionPlanningInput`,
`BatchExecutionPlanningRequest` and
`PreparedProviderBatchExecutionPlan`, and the registered WELearn
`TaskExecutionCapability` implements both batch hooks. Its
`welearn.atomic-batch-request.v1` input is deny-unknown, versioned, locally
bounded and provider-namespaced; it freezes Course, atomic flow, ordered Unit
selection, expected child, the complete Fanyuchang target vector or Auto's
complete sampled aggregate budget. Fresh prepare validates the resolved
runtime settings and exact atomic capability set, lists Courses once, performs
one all-or-nothing Unit/SCO scan that retains empty Units, rebuilds the batch
once and returns the private parent pair plus every ordered Core child through
the shared prepared-result validator. Deterministic restore has no inventory
dependency and reconstructs the complete child plan only from the resolved
encrypted parent pair.
...
and the 8,192-child maximum are covered. Core now binds that membership value,
the v2 target-authorized parent pair and every child artifact through the same
parent Attempt and Requested draft. Neither the snapshot nor the draft alone is
scheduling or mutation permission.
...
remaining plan work is child activation and the composite dispatch layer,
...
`BatchExecution`/Attempt, `34e64db` freezes parent runtime settings, `7aaadd5`
materializes the complete ordered child plan and `7d7ff81` creates the Requested
child Execution drafts with full steps/settings/artifact/mapping. WELearn can
now restore each exact child directly from resolved parent + durable ordinal +
the ordinary reconstructed `ExecutionRequest`; Main need not rebuild a generic
child plan. The remaining dependency is activation/job creation and dispatch
that resolves the same parent Attempt and calls the materialized execution or
recovery entry without rescanning.
