...
| TaskDetail | Fresh Course/Unit/SCO inventory | FromScratch | Re-lists Courses and re-runs one complete Course scan, then requires exactly one matching stable SCO identity, identical normalized Task snapshots in both public and nested detail views, and the exact versioned fingerprint derived from that snapshot; mixed or stale injected/restored details fail closed before execution; live pending |
