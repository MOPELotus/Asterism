...
- UAI clean-room 上游审计、Password / ImportedToken 认证编排、Core scoped 持久会话解析及 fixture-only CourseResource / Unit / Section / Micro / Group 有界解析；原生 Password JSON 登录和只读 user-info JWT 校验已复用共享网络策略，openid/JWT 作为原子加密 CompositeSession 保存，滑块验证码进入 HumanRequired；Course 点数、Task completion 和 Learning Duration 保持独立，自动续登、原生 Course/Task 网络读取与时长上报尚未接入；
...
| 第一批 | `chaoxing`、`welearn`、`uai`、`cidaren` | 开发中（Chaoxing 已建立 Password → Cookie → Course / Chapter / Resource / Work / Exam 的开发验证链路；WELearn 已建立 Password/OIDC → Cookie validation → Course / Unit / SCO → CMI progress 的原生只读边界；UAI 已建立 CourseResource → Unit / Section / Micro / Group 的 fixture-only 解析边界；均未完成真实账号验证） |
