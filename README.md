...
- UAI clean-room 上游审计、Password / ImportedToken 认证编排、Core scoped 持久会话解析及 CourseResource / Unit / Section / Micro / Group 有界解析；原生 Password JSON 登录、只读 user-info JWT 校验和 CourseInventory 已复用共享网络策略，openid/JWT 作为原子加密 CompositeSession 保存，滑块验证码进入 HumanRequired；Course 点数、Task completion 和 Learning Duration 保持独立，自动续登、原生 Task/progress 读取与时长上报尚未接入；
