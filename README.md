...
- Cidaren 已建立 ImportedToken → Core scoped stored `UserToken` → bounded `Student/Main` validation → signed complete `ClassTask/PageTask` pagination → Course / learning / test Task 的原生只读边界；稳定任务身份使用 `release_id`，`task_id=-1` 只作 fresh observation，进度、完成、过期、分数和 raw time 独立保留；当前只声明 Authentication、CourseInventory、TaskInventory，微信 OAuth/Capture、Duration、答案、执行和真实账号验证均未接入；
