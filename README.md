...
- WELearn clean-room 已覆盖三个 pinned donor 的 Password/OIDC、ImportedCookie 与 Capture-assisted Cookie 认证，Core 持久会话解析/续登，Course/Unit/SCO inventory、fresh TaskDetail/CMI Progress、canonical DurationRead，fixed/random DurationReport，以及直接 `start -> setscoinfo -> save` completion/progress/score ResourceExecution；ResourceExecution 使用冻结设置重算同一目标并 fresh CMI 验证 exact completion/progress/score，DurationReport 保持独立的 preservation/time-change 验证且歧义写入不重放；平台/账号/任务级运行设置已接入。验证码/短信在没有 donor solver 协议证据时保持 typed HumanRequired，而不是能力政策裁剪；Unit/全课程批量选择与预算分配、Capture acquisition-method 精确落库、真实账号端到端和脱敏 live fixture 验证继续推进；
...
- Cidaren 已覆盖 ImportedToken/Composite 会话、bounded `Student/Main` validation、Course/class/study inventory、fresh detail/progress/duration、当前 `jv=99` HKDF/AES-GCM Question/Answer/SubmissionBuild，以及 Provider-private Start/Verify/Submit/Skip/ChoseWord 状态机与 fresh SubmissionVerify；V2 微信 OAuth code exchange、P-256/ECDH/HKDF/AES-GCM token 解密和 Browser policy 亦已实现。剩余共享工作包括 PC 微信 XWeb callback/Capture artifact 的一次性持久消费、耐久 QuestionSession/Attempt continuation、BrowserBridge 执行器和真实账号验证；这些继续处于当前实现范围；
