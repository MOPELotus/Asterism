...
- UAI clean-room 上游审计、Password / ImportedToken 认证编排及 fixture-only CourseResource / Unit / Section / Micro / Group 有界解析；openid/JWT 作为原子加密 CompositeSession 保存，滑块验证码进入 HumanRequired；Course 点数、Task completion 和 Learning Duration 保持独立，原生网络与时长上报尚未接入；
