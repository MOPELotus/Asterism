...
- UAI clean-room 上游审计、Password / ImportedToken 认证编排、Core scoped 持久会话解析与原子自动续登，以及 CourseResource / Unit / Section / Micro / Group 有界解析；原生 Password JSON 登录、只读 user-info JWT 校验、Course/Task inventory、per-Unit `TaskProgressRead` 与独立 `DurationRead` 已复用共享网络策略，并组合为默认关闭、要求 SecretStore 的 daemon Development 注册入口；Task/Progress 均先重新绑定 fresh CourseResource detail，时长读取则用 fresh user identity 绑定 CourseResource / Unit / Group 后只接受 study-record 接口明确以秒计的 duration；openid/JWT 作为一个 CompositeSession 替换，ManualImport JWT 不可自动续期，滑块验证码进入 HumanRequired；Group 仅在 pass/pass2/perm 均为 1 时完成，progress 原始 duration 仍不换算秒数；DurationReport 与执行尚未接入；
