...
- 显式 Score Improvement opt-in 现只接受已验证 Completed/Passed 基线和不早于该完成事实的同 owner/account/Task 历史结果，冻结 history import/result digest、Provider 计分保留策略、剩余次数与截止时间，并仅允许这些远端事实收紧 Core 策略；创建 workflow 不调用 Provider、不创建 Execution/Attempt，旧的无 authority workflow 也不能启动 retake；
...
- Chaoxing 能力级上游审计、独立 Chapter / Resource / Work / Exam TaskInventory、按稳定身份重新发现并经 Core 校验的 TaskDetail / Progress API 与 CLI、四类任务的模块化只读进度复核、Chapter Work 与独立 Work / Exam 题面解析，以及 Work/Chapter Work 的原生 Submission 链均已有离线覆盖；Exam Pending/goTest 现走独立的 cover -> durable one-shot start -> 完整 mobile preview -> ordered answer saves -> final submit 链。每个随机签名请求先冻结并持久化 digest，成功保存只旋转单调 `enc`/计时 continuation，末次提交只形成 Receipt，再由 fresh Exam inventory 确认 Completed；临时保存歧义不重放，最终提交歧义也只允许只读 Completed 恢复，列表完成不冒充逐题正确。二维码认证已接入 Core 加密续态、串行轮询、终态恢复和确定性凭据提交，扫码成功后还须以 fresh Course list 重新验证身份；验证码/人脸/考试码分支进入 typed BrowserRequired。Document / Read / Video 原生执行、有界 Work 详情状态复核、Cookie 自动续登与显式开发验证入口均保持 Development；Live、QR 真实账号验证、BrowserBridge/Capture fallback、Exam 结果细节及其他 donor 已知能力继续纳入当前开发；
...
- UAI clean-room 已覆盖 Password / ImportedToken / Capture 认证编排、Core scoped 持久会话解析与原子自动续登、CourseResource / Unit / Section / Micro / Group inventory、fresh detail/progress/duration、完整加密 Question/Answer/Submission 与已验证 ResourceExecution；带媒体的完整题组现通过一个 Task/Question/fingerprint 绑定的加密 QuestionSession artifact 交给 Provider-native AnswerResolve，并以 issue-before-send 的精确请求摘要执行一次原生 media submission。Provider-private Browser residence、discussion、upload 和媒体边界继续映射 donor 真实语义；共享 BrowserBridge 执行器、外部下载/模型/转写编排、DurationReport 的浏览器执行和真实账号验证仍在推进，不把共享 Core Gap 当成 Provider 结束理由；
- Cidaren 已覆盖 ImportedToken/Composite 会话、bounded `Student/Main` validation、Course/class/study inventory、fresh detail/progress/duration、当前 `jv=99` HKDF/AES-GCM Question/Answer/SubmissionBuild，以及 Start/Verify/Submit/显式 Skip/ChoseWord 状态机与 fresh SubmissionVerify；状态机已接入耐久 QuestionSession operation ledger，区分同题 Continue、下一题原子快照切换和无需伪 continuation 的末题 Submitted。随机 marker 微信 OAuth、hash-only pending/CAS、一次性 callback 消费、P-256/ECDH/HKDF/AES-GCM token 解密和 Browser policy 亦已离线覆盖。剩余工作包括替代 donor XWeb Capture helper 执行、BrowserBridge 执行器和真实账号验证；这些继续处于当前实现范围；
