ALTER TABLE qq_formal_notification_deliveries RENAME TO qq_notification_deliveries_v107;
...
CREATE TABLE qq_formal_notification_deliveries (
    ...
    notification_kind TEXT NOT NULL CHECK (notification_kind IN (
        'confirmation_due', 'deadline_missed', 'execution_progress',
...
SELECT id, task_id, execution_id, user_id, qq, notification_kind, deduplication_key,
       state, attempts, claimed_at, next_attempt_at, delivered_at, last_error,
       created_at, updated_at
FROM qq_notification_deliveries_v107;

DROP TABLE qq_notification_deliveries_v107;
