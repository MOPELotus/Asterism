...
- **D-003：四平台先用只读链路建立事实，再按 donor 能力接入执行。** 四个 Adapter 均接到账号、
  课程和任务扫描；题目扫描只用于真实需要逐题处理的 Provider。WELearn/UAI 的完成与时长
  阶段保持 donor 私有且彼此分离，Chaoxing/Cidaren 的逐题链路按各自题型与确认策略进入
  execution/mutation；只读检查点不再是执行功能的永久停止线。
