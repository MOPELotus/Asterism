...
许可证尚未确认的 donor 不进入默认便携包。运行时按固定 revision 解析本地 donor：优先应用目录、
配置指定目录和数据目录；找不到时只有在显式允许网络后才使用系统 Git 的 detached checkout，
再回退到固定 commit ZIP。下载后必须通过入口 SHA-256 校验，禁止跟踪浮动分支；未安装 donor
只使对应 Provider 显示不可用，不阻断其他平台。
