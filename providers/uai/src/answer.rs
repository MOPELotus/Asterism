...
fn parse_answer_entry(entry: &Value, question: &Question) -> ProviderResult<AnswerCandidate> {
    ...
    let answer = match question.kind {
        ...
        QuestionKind::ShortAnswer => NormalizedAnswer::Texts(short_answer_texts(entry)?),
        _ => {
...
fn short_answer_texts(entry: &serde_json::Map<String, Value>) -> ProviderResult<Vec<String>> {
    let analysis = entry
        .get("analysis")
        .and_then(Value::as_str)
        .filter(|value| !value.is_empty())
