...
/// Parses one complete donor-observed mobile Work Question document without
...
pub fn parse_work_question_page(html: &str) -> ProviderResult<Vec<ParsedChaoxingQuestion>> {
    parse_question_page(html, QuestionPageKind::WorkMobile)
}
...
fn parse_question_page(
    ...
    let node_selector = selector(match page_kind {
        QuestionPageKind::WorkMobile => "div.Py-mian1",
...
fn parse_question_node(
    ...
    let type_input = node
        .select(&selector(match page_kind {
            QuestionPageKind::WorkMobile => "input[id^='answertype']",
            QuestionPageKind::ExamMobile => "input[name^='type']",
        }))
        .next()
        .ok_or_else(|| protocol_drift("Chaoxing Question has no type input"))?;
    let remote_id = match page_kind {
        QuestionPageKind::WorkMobile => type_input
            .value()
            .attr("id")
            .and_then(|value| value.strip_prefix("answertype")),
        QuestionPageKind::ExamMobile => node
    ...
    let title = node
        .select(&selector(match page_kind {
            QuestionPageKind::WorkMobile => "div.Py-m1-title",
    ...
    let options = parse_options(node, page_kind)?;
    if matches!(
        kind,
...
fn parse_options(
    ...
    let option_selector = selector(match page_kind {
        QuestionPageKind::WorkMobile => "li.more-choose-item",
...
fn normalized_stem(
    ...
    let mut fragments = title
        ...
        .collect::<Vec<_>>();
    match page_kind {
        QuestionPageKind::WorkMobile if fragments.len() > 2 => {
            fragments.drain(..2);
        }
        QuestionPageKind::ExamMobile => {
...
mod tests {
    ...
    fn work_mobile_questions_are_typed_bounded_and_answer_free() {
        let parsed = parse_work_question_page(WORK_MIXED).unwrap();
    ...
    fn login_and_unknown_documents_are_not_treated_as_empty_question_sets() {
        let authentication =
            parse_work_question_page("<form action='/fanyalogin'><input name='uname'></form>")
                .unwrap_err();
